...
def rule2(self, input): #
    ...
    two = re.search(r'how much (.+) does \b(?:a|an|the)\b (.+) have?', input)
...
def rule3(self,input): # 2 var
    ...
    three = re.search(r'the (.+) (.+) of \b(?:a|an|the)\b (.+)', input)
...
def rule10(self,input):
    """Rule 10 applies if the user asks what ELIZA's favorite car is"""
    ten = re.search(r'(?:what) (?:.*) (favorite+)', input)
    if ten:
        make, car = randomCar()
        return True, 'My ' + ten[1] +' sports car is the ' + make + ' ' + car + '.' + ' What is your ' + ten[1] +' car?'
    else:
        return False, None
...
def rule12(self,input):
    """Enter function"""
    twelve = re.search(r'blah blah (.+)', input)
    if twelve:
...
def rule13(self,input):
    """Enter function"""
    thirteen = re.search(r'blah blah (.+)', input)
    if thirteen:
